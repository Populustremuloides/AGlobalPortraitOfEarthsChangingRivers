...
df = pd.read_csv("ml_all_years_data.csv")
df = df.transpose()
...
class LinearNetwork(nn.Module):
    def __init__(self, in_dim = 6778, out_dim = 6778, compression_level=3):
...
for c_level in range(1,4):
    ...
    for x, catchment in loader2:
        ...
        dataDict["catchment"].append(catchment[0])
        #print(encoding)
        if c_level >= 1:
    ...
    outDf.to_csv("ml_through_time_encodings_" + str(c_level) + ".csv")
    
    with open("ml_through_time_encodings_loss_" + str(c_level) + ".csv", "w+") as oFile:
        oFile.write("loss")
