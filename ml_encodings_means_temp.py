...
noiseToAdd = np.random.normal(size=len(list(df["gord"])))
noiseToAdd = noiseToAdd * 0.18
print(noiseToAdd)
gords = list(df["gord"])
print(gords)
#quit()
gords = np.add(gords, noiseToAdd)


plt.scatter(df["MeanPrecAnn"],df["MeanTempAnn"], c=colors, alpha=1, s=2)
plt.xlabel("mean annual precipitation")
plt.ylabel("mean annual temperature")
plt.title("Spectral Numbers across Spatial Features")
plt.show()



plt.scatter(gords,df["MeanTempAnn"], c=colors, alpha=0.8)
