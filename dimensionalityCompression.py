...
df = pd.read_csv("universallyAligned_powers.csv", index_col=0)

dataset = CustomImageDataset(df)

# Extend the torch.Module class to create your own neural network
class LinearNetwork(nn.Module):
    def __init__(self, in_dim = 1101, out_dim = 1101, compression_level=3):
...
for c_level in range(1,4):
    ...
    outDf.to_csv("ml_encodings_" + str(c_level) + ".csv")
    
    with open("ml_encodings_loss_" + str(c_level) + ".csv", "w+") as oFile:
        oFile.write("loss")
