            ...
            if "localWaterYear" in file: # FIXME:
                ...
                for cat in list(df.columns)[1:]:
                    if str("X" + cat) in catToArea.keys():
                        ...
                        newDict["X" + cat] = [np.mean(flowData)]
...
dataDict = {
        "mean_annual_specific_discharge":[],
        "year":[],
        "catchment":[],
        "category":[]
        }
...
def updateDataDict(dataDict, df, year, category):
    for col in df.columns:
        dataDict["mean_annual_specific_discharge"].append(df[col][0])
...
outDf.to_csv("specificDischargeThroughTime.csv")
